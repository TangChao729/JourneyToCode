...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        # a tuple of current position, and current goals left
    ...
    def getStartState(self):
        ...
        return (self.start, tuple(self.foodGrid.asList()))
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = state[0]
            foodToEat = list(state[1])
            dx, dy = Actions.directionToVector(action)
            next_x, next_y = int(x + dx), int(y + dy)

            if (x, y) in self.foodGrid.asList():
                if (x, y) in foodToEat:
                    foodToEat.remove((x, y))
                    foodToEat.sort()

            if not self.walls[next_x][next_y]:                
                nextState = ((next_x, next_y), tuple(foodToEat))
                cost = 1
                successors.append((nextState, action, cost))    
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = state[0]
            foodHasEat = list(state[1]).copy()
            dx, dy = Actions.directionToVector(action)
            next_x, next_y = int(x + dx), int(y + dy)

            if (x, y) in self.foodGrid.asList():
                if (x, y) not in foodHasEat:
                    foodHasEat.append((x, y))
                    foodHasEat.sort()

            if not self.walls[next_x][next_y]:
                nextState = ((next_x, next_y), tuple(foodHasEat))
                cost = 1
                rev_action = Actions.reverseDirection(action)
                successors.append( ( nextState, rev_action, cost) )     
...
def bidirectionalFoodProblemHeuristic(state, problem):
    ...
    # Manhattan Distance to Furthest Goal
    position = state[0]
    foodList = state[1]
...
def bidirectionalFoodProblemBackwardsHeuristic(state, problem):
    ...
    # Manhattan Distance to Furthest Goal or Pacman
    position = state[0]
    foodHasEatList = state[1]

    # differ to forwards max manhanttan distance, this looking for food have not being eaten
    allFood = problem.foodGrid.asList().copy()
    for food in list(foodHasEatList):
        allFood.remove(food)
    
    # plus the pacman position
    allFood.append(problem.start)
    distance = []

    for food in list(allFood):
        distance.append(util.manhattanDistance(position,food))
